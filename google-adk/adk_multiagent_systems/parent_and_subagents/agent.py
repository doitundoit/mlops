...
def save_attractions_to_state(
    ...
    # When the tool is run, ADK will create an event and make corresponding updates in the session's state.
    tool_context.state["attractions"] = existing_attractions + attractions

    # A best practice for tools is to return a status message in a return dict
    return {"status": "success"}

def remove_attractions_to_state(
    tool_context: ToolContext,
    attractions: List[str]
) -> dict[str, str]:
    """Removes the list of attractions to state["attractions"].
    ...
    # Update the 'attractions' key with a combo of old and new lists.
    # When the tool is run, ADK will create an event and make corresponding updates in the session's state.
    tool_context.state["attractions"] = [attraction for attraction in existing_attractions if attraction not in attractions]
...
attractions_planner = Agent(
    ...
    instruction="""
        ...
        - If they ask to view the list, provide a bulleted list of {attractions?} and then suggeset some more.
        """,
    before_model_callback=log_query_to_model,
    after_model_callback=log_model_response,
    # When instructed to do so, paste the tools parameter below this line
    tools=[save_attractions_to_state, remove_attractions_to_state]
    )
